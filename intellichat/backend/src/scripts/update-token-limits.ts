...
async function updateTokenLimits() {
  try {
    ...
    console.log(`✅ Updated ${result.modifiedCount} users with new token limit: ${NEW_TOKEN_LIMIT.toLocaleString()}`);

    // Show current user stats
    const users = await User.find({}).select('email subscription.tokensLimit subscription.tokensUsed');
    ...
    users.forEach((user: any) => {
      ...
      console.log(`   Tokens: ${used.toLocaleString()} / ${limit.toLocaleString()} (${percentage}%)`);
    });
    console.log('─'.repeat(80));
